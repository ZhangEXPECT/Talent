...
import com.example.talent.dao.HouseDao;
...
//    }
    ...
    @Override
    public ResultVO changeStatusById(Integer houseId) {
    ...
    public ResultVO add(House house) {
        ...
        if (this.houseDao.queryCondition(house1).isEmpty()) {
            ...
            return new ResultVO(ResultEnum.ADD_FAIL, "错误！！无法重复发布！！");
        }
    }
    ...
    public ResultVO update(House house) {
        ...
        house1.setHouseId(house.getHouseId());
        if (this.houseDao.queryCondition(house1).isEmpty()) {
            return new ResultVO(ResultEnum.UPDATE_FAIL, "该房源不存在！！修改错误！！");
        } else {
            this.houseDao.update(house);
     ...
     */
    @Override
    public ResultVO delete(Integer houseId) {
        House house1 = new House();
