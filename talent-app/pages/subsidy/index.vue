	...
	<view class="application">
		...
		<view class="form" v-if="step===1">
			...
			<u-form :model="unitForm" ref="unitForm" :error-type="['message']">
				<u-form-item label="居住地址" label-width="200" :label-style="label_style" required>
					<u-input v-model="unitForm.unitName" />
				</u-form-item>
				<u-form-item label="所属区域" label-width="250" :label-style="label_style" required>
					<u-input v-model="unitForm.unitCode" />
				</u-form-item>
				<u-form-item label="房东姓名" label-width="200" :label-style="label_style" required>
					<u-input v-model="unitForm.unitContact" />
				</u-form-item>
				<u-form-item label="联系方式" label-width="200" :label-style="label_style" required>
					<u-input v-model="unitForm.unitPhone" />
				</u-form-item>
				<u-form-item label="身份证号" label-width="200" :label-style="label_style">
					<u-input v-model="unitForm.intentHouseType" type="select" @click="showHouseTypeList=true" />
				</u-form-item>
				<u-form-item label="入住时间" label-width="200" :label-style="label_style" required>
					<u-input v-model="unitForm.intentAera" type="select" @click="showAeraList=true" />
				</u-form-item>
				<u-form-item label="到期时间" label-width="200" :label-style="label_style" required>
		...
		<view class="form" v-if="step===3">
			...
			<u-form :model="fileForm" ref="fileForm" :error-type="['message']">
				...
				<u-form-item label="劳动合同上传" label-position="top" label-width="800" :label-style="label_style" required>
					<u-upload width="160" v-model="fileForm.otherImg" :show-progress="true" height="160"
						:action="action" :auto-upload="true" @on-remove="onRemove" :file-list="fileList"
						@on-success="successUpload3">
					</u-upload>
				</u-form-item>
				<u-form-item label="租房合同" label-position="top" label-width="800" :label-style="label_style" required>
					<u-upload width="160" v-model="fileForm.otherImg" :show-progress="true" height="160"
						:action="action" :auto-upload="true" @on-remove="onRemove" :file-list="fileList"
		...
		<view class="forward" v-if="step==3">
			<button @click="step=1" type="primary">上一步</button>
		</view>
		<!-- 下一步 -->
		<view class="next">
	...
	export default {
		...
		methods: {
			...
			successUpload1(data, index, name) {
				if (data.code == 200) {
					this.fileForm.graduateImg = data.data;
				}
			},				
			...
			successUpload3(data, index, name) {
				if (data.code == 200) {
					this.fileForm.employedContract = data.data;
				}
			},
			successUpload4(data, index, name) {
				if (data.code == 200) {
					this.fileForm.houseContract = data.data;
				}
			},
			successUpload5(data, index, name) {
				if (data.code == 200) {
