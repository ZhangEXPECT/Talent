  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          that.logining = true
          if (this.ruleForm.code !== 'M8K2') {
            this.$message.error("验证码错误！")
          }
          let param = { 
            adminAcc: this.ruleForm.username,
          ...
          login(param).then(res => {
            ...
            if (res.status==200) {
              if (that.rememberpwd) {
                //保存帐号到cookie，有效期7天
                setCookie('user', that.ruleForm.username, 7)
                //保存密码到cookie，有效期7天
                setCookie('pwd', that.ruleForm.password, 7)
              } else {
                delCookie('user')
                delCookie('pwd')
              }
              ...
              setTimeout(() => {
                ...
                that.$router.push({ path: '/info/Policy' })
              }, 1000)
            } else {
    ...
    getCode() {
      this.$set(this.ruleForm, "codeimg", "/static/code.png")
    }
  }
}
</script>
